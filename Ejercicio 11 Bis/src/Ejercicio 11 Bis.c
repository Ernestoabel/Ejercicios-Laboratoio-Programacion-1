...
int main(void) {
	...
	int posicionVacia,eleccionDos,modificar,elegirModificar,cantidadSueldosMasAltos=0,validarModificacfion,contadorSector=0;
	int elegirSector;
	char eleccionUno;
        ...
        switch(eleccionUno){
            ...
            case '4':
            	do{
            	    ...
            	    "\n3.Para saber el sector con mas empleados","\n4.Listar empleados por sector",
					"\n5.Mostrar el sector y su cantidad de empleados",
					"\n6.Calcular y mostrar promedio de sueldo por sector","\nElija una opcion: ");
            	    if(modificar==1){
                	...
                	if(modificar==4){
                		mostrarEmpleadosPorSector(empleados,TAM,listaSector);
                	}
                	if(modificar==5){
                		elegirSector=contadorPorSector(empleados,TAM,&contadorSector);
                		mostrarCantidadEmpleadosSector(empleados,TAM,listaSector,elegirSector,contadorSector);
                	}
                	if(modificar==6){
                		totalSueldos=sacarPromedioEstructurasPorSector(empleados,TAM,&resultado);
                		printf("\nEl total de sueldos es %.2f y su promedio es %.2f",totalSueldos,resultado);
                	}
